...
**Current version:** `0.8.391` (from `project(PIPBONG VERSION 0.8.391)` in `CMakeLists.txt` → `PipbongVersion.h` → `QCoreApplication::applicationVersion()`)
...
| Profile memo            | `%LOCALAPPDATA%/PIPBONG/PIPBONG/profiles/{profileId}/memo.txt` — plain UTF-8 text per profile; included in `.pipbong` package mirror; dialog placement in `QSettings` `memo/placement/*` (legacy `memo/geometry`); per-profile open/closed state in `memo/open/{profileId}`                                                                                                                                          |
...
- **Persistence:** profile workspace `memo.txt` (travels with `.pipbong` export); window placement in `QSettings` `memo/placement/*`; per-profile open/closed state in `memo/open/{profileId}` (restored on startup and profile switch).
...
| **R5** | MainWindow decomposition | **Partial** | 0.8.391 | R5.2 `sessionForEngine`, session erase/teardown on coordinator; no direct `m_runSessions.erase` in `MainWindow`; ~8515 lines; R5.5 manual §8.17 pending user |
...
| **R5.2** `RunLifecycleCoordinator` (new) | Owns: `startFeatureRun`, `stopFeatureRun`, `finishRunSession`, `m_runSessions` map mutations, `applyRunUiState` orchestration | **Partial** — map storage on `MainWindow`; mutations via `tearDownAndEraseSessionEntry` / `eraseSessionEntryAndRefreshRunUi` / `sessionForEngine` (v0.8.391); hold-burst on coordinator (v0.8.389–0.8.390) |
...
- `RunLifecycleCoordinator`: `sessionForEngine`, `holdSessionBlocksNewPhysicalStart`, `tearDownAndEraseSessionEntry`, `eraseSessionEntryAndRefreshRunUi`; all `MainWindow` session-map erases route through coordinator (§8.21 R5.2).
...
- Hold-burst coalesced UI scheduling (`prepareForegroundForHoldBurst`, hold start/end flush, deferred burst side effects, hold feature start/end queues) moved to `RunLifecycleCoordinator`; `MainWindow` thin delegates (`RunLifecycleCoordinator`, `MainWindow`).
...
- **`RunLifecycleCoordinator::applyRunUiState`**: feature list run chrome, workflow run button, status bar, trigger restore deferral, and burst-side-effect scheduling moved from `MainWindow::applyRunUiState` (`RunLifecycleCoordinator`, `MainWindow`).
- **`RunLifecycleCoordinator::requestRunUiRefresh`**: run UI debounce/coalesce logic (was `MainWindow::updateRunUiState` body); `MainWindow::updateRunUiState` forwards only.
- AGENTS.md §8.21: R5.3 UI refresh facade done (code); `MainWindow.cpp` ~8727 lines.

## [0.8.385] - 2026-07-30

### Changed

- **`RunLifecycleCoordinator::startFeatureRun`**: full feature-run start orchestration (validation, foreground gate, reconcile, prepare, launch); `MainWindow::startFeatureRun` only wraps stutter scope (`RunLifecycleCoordinator`, `MainWindow`).
- AGENTS.md §8.21: R5.2 start path complete; `MainWindow.cpp` line count updated.
...
- **`RunLifecycleCoordinator::prepareNewSession`**: new-feature session shell (engine connect, trigger/hold/repeat fields, hold-key-tap fast-path flag) before `m_runSessions.emplace` (`RunLifecycleCoordinator`, `MainWindow::startFeatureRun`).
...
- **`PIPBONGWorkflowDryRunSim`**: loop-region scenarios (`LastMatchFailed` / `LastMatchSuccess`, single-iteration exit) in `WorkflowDryRunSim.cpp` (AGENTS.md §8.21 R6.2).
- **`scripts/run-workflow-dry-run-postbuild.ps1`**: runs workflow dry-run sim after PIPBONG link; skip with `PIPBONG_SKIP_WORKFLOW_DRY_RUN=1`; CMake `PIPBONG_RUN_WORKFLOW_DRY_RUN_ON_BUILD` (default ON).

## [0.8.377] - 2026-07-29

### Added

- **`SessionRunPolicy::countActiveRepeatSessions`**: counts active `repeatSession` sessions for multi-hold UI coalesce (`SessionRunPolicy.*`, `MainWindow::concurrentActiveRepeatSessionCount`, `SessionRunPolicySim` `count_active_repeat_sessions`).
- **`PIPBONGWorkflowDryRunSim`**: links `WorkflowRunner` and runs a wait-only workflow smoke test (AGENTS.md §8.21 R6.2); `WorkflowDryRunSim.cpp`, expanded `CMakeLists.txt` target sources.

### Fixed

- **`scripts/run-workflow-dry-run.ps1`**: pass `-RepoRoot` to `Prepare-IncrementalBuildEnvironment` / `Ensure-BuildTreeConfigured` (was failing at runtime).
...
- Hold / N-repeat workflow loops no longer re-run `refreshSessionCaptureTarget` (HWND lookup + global `ScreenCapture` sync) on every iteration when `lockedCaptureTargetTitle` is already set — roadmap R3.3 (`MainWindow::sessionCaptureTargetTitleW`, `launchWorkflowRun`, `syncRunSessionContext`).

## [0.8.371] - 2026-07-29

### Added

- `SessionRunPolicySim` multi-session roadmap scenarios (trigger watch + hold burst, trigger action + watch, background-style session mix) — R1.3 (`SessionRunPolicySim.cpp`).

### Changed

- AGENTS.md §8.21 R3.2 audit note: `onBlockImageFindAttempt` does not mutate global `ScreenCapture` on the poll UI callback path.

## [0.8.370] - 2026-07-29

### Changed

- Abandoned workflow engines: `pruneAbandonedEngines` performs at most one `stopAndWaitBounded` per scheduled prune tick and re-schedules when more engines need teardown — avoids stacking many 1 ms GUI-thread waits when several sessions stop together (`MainWindow`).

### Fixed

- (Roadmap R3.1) Document session locked capture title and worker poll path in `FeatureRunSession` header comment.

## [0.8.369] - 2026-07-29

### Added

- Track always-applied `.cursor/rules/hold-mode-latch.mdc` in git (hold continuation latch policy; AGENTS.md §8.20 already referenced the file since v0.8.362).
...
- Task-close user summary (§9.7, `task-close-summary.mdc`): mandatory **5-line** block adds **앞으로 할 일** (next roadmap / follow-up) alongside **이번에 할 일** (immediate user actions); pointers updated in `brief-korean-replies.mdc`, `ai-governance.mdc`, §9.5.
...
- Feature list UI terminology **묶음** → **그룹** (toolbar button, badges, menus, dialogs); removed list hint label and long group-row/list tooltips (`FeatureListPanel`).
- Default new feature group name **그룹** / **새 그룹** (`FeatureGroup`, `JsonSerializer` load fallback).
...
- Feature list **묶음** UX: single-click group header toggles expand/collapse; clearer group row chrome (chevron badge, **묶음** pill, collapsed hint); muted hint line under toolbar; member feature rows indented; drag feature(s) onto a group header to assign and move into the group; drag group header moves all members together (`FeatureListPanel`, `FeatureListWidget`, `FeatureListItemRoles.h`, `ReorderableListWidget::clearActiveDropChrome`).
...
- Profile foreground auto-switch: `ForegroundWindowMonitor` polls `GetForegroundWindow` every 250 ms when WinEvent delivery is missed; warns when `SetWinEventHook` fails (`ForegroundWindowMonitor`).
- Profile resolution prefers **foreground HWND** match against each profile’s resolved main/sub target window before process-path/title heuristics (`ProfileForegroundResolver::profileIdForForegroundHwnd`, `MatchKind::ForegroundHwnd`).

## [0.8.336] - 2026-07-27

### Changed

- GUI hang reporting: `pending.txt` written immediately when the crash folder is created (before heavy stack/minidump I/O); hang threshold 6 s → 4.5 s; skip hang minidump; clear `g_writingCrashReport` after hang capture; recover `g_guiHangReported` when the GUI heartbeat returns; topmost Korean `MessageBox` after hang report plus `QProcess::startDetached` fallback for `--crash-report` viewer (`CrashReporter`).
- Startup crash UI: `startupReportSummary()` shows the latest recent crash folder (72 h) when `pending.txt` is missing; `crash/lastDismissedReportFolder` in `QSettings` (`CrashReportDialog`, `CrashReporter`).

## [0.8.335] - 2026-07-27

### Fixed

- Profile foreground auto-switch: start `ForegroundWindowMonitor` after profiles load (avoids early resolve on empty manifest); heal linked exe paths on all matching non-default profiles before resolve; re-run foreground coordinator when PIPBONG window activates after WinEvent skip (`ProfileForegroundResolver::healProfileBindingsFromForeground`, `ProfileSwitchCoordinator`, `MainWindow`, `ForegroundWindowMonitor`).
...
- **홀드** mode feature-list running glow now tracks physical key hold only (`holdRunActive`), not lane/engine teardown — glow turns off immediately on hotkey release via lightweight `refreshFeatureListHoldVisuals()` bypassing coalesced `updateRunUiState` debounce (`MainWindow::isFeatureRunHighlighted`, `buildFeatureListRunVisualKinds`, `onHotkeyHoldEnded`, `startFeatureRun`).
...
- Foreground profile auto-switch now reliably falls back to the **default** profile when the focused window matches no linked profile: `resolveProfileIdForForeground` no longer returns empty for titleless/unmatched HWNDs; `syncProfileToForegroundWindow` treats empty resolve as default and commits default fallback via `executeProfileSwitch` after the stable delay (not blocked by multi-hold `requestAutoProfileSwitch` gate); default fallback no longer aborts on empty window title (`MainWindow`).
...
- Q/W/E/R simultaneous **홀드** lag: keyboard hold KEYUP defers hold-end up to 8×32 ms rechecks so multi-key rollover ghost releases no longer tear down sessions (`HotkeyManager::scheduleDeferredKeyboardHoldEnd`); hold-key-tap fast path runs coalesced start UI once per session (`holdKeyTapStartUiDone`); profile auto-switch is skipped while two or more repeat/hold sessions are active (`MainWindow::requestAutoProfileSwitch`).
...
- Workflow block list **요약** for **마우스** blocks with Ctrl/Alt/Shift modifiers shows the modifier prefix (e.g. `Ctrl+Shift+클릭·직전`) via `ClickBlock::listDetailSummary` (`ClickBlock`).
...
- Workflow block list row reorder drag no longer crashes with `EXCEPTION_STACK_OVERFLOW`: skip **기준/감지** `setCursor`/`unsetCursor` while the left mouse button is held (re-entrant cursor events during drag); use guarded `syncScoreColumnResizeCursor` instead of `unsetCursor` (`BlockListWidget`).
...
- Workflow block list **기준/감지** threshold horizontal drag no longer starts row reorder drag or freezes the UI: score-column press grabs the mouse, suspends `InternalMove`, blocks `startDrag` while adjusting, and uses app-level move/release handling (`BlockListWidget`).
...
- Unified per–run-mode theme across the app via shared `FeatureRunModeTheme` helpers: workflow status bar mode chip, **기능 편집** mode preview chip and combo accent, workflow block-list running highlight, and trigger watch/cooldown defaults now use the same **홀드** amber / **무한 반복** blue / **트리거** jade-teal / **N회 반복** violet palette (`FeatureRunModeTheme.cpp`, `WorkflowRunStatusBar`, `FeatureEditDialog`, `BlockListWidget`, `TriggerListAnimationRenderer`).
...
- Trigger mode **감시** no longer stalls without ImageFind poll attempts after a main↔sub capture-target round-trip (e.g. in-game on main, then back to launcher sub): `ensureTriggerMonitorEnginesRunning` relaunches the monitor worker when the session UI still shows **감시** but the engine stopped; capture-scope migration and foreground changes schedule the same relaunch (`MainWindow`).
...
- **메모장** window position/size no longer jumps on reopen or profile switch: frameless dialog stores explicit screen + frame geometry (`memo/placement/*`), restores only on first show per instance, persists after drag/close/hide; migrates legacy `memo/geometry` (`MemoDialog`).
...
- **Workflow run status bar** shows the selected feature's run mode chip (**홀드** / **무한 반복** / **트리거** / **N회 반복 ×N**) beside the feature name (`WorkflowRunStatusBar`, `WorkflowEditorPanel`).

### Fixed

- **중앙 고정** did not move the window when a profile **서브 대상 창** was configured: center-pin sync now refreshes capture bindings, resolves the auto main/sub target title, skips minimized HWNDs, and re-finds before `SetWindowPos` (`MainWindow::prepareCenterPinTargetWindow`, `TargetWindowCenterPin`).
...
- Per-feature **캡처 대상** scope in **기능 편집**: **자동 (메인·서브)** / **메인 대상 창만** / **서브 대상 창만**; JSON `captureTargetScope` (`Auto` / `MainOnly` / `SubOnly`, omitted when `Auto`); run session locks capture title via scoped `resolveRunCaptureTargetTitleW` (`FeatureCaptureTargetScope`, `Feature`, `FeatureEditDialog`, `JsonSerializer`, `MainWindow`).
