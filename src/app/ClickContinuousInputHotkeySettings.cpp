...
constexpr auto kPrefix = "hotkeys/clickContinuousInput";

HotkeyBinding defaultBinding() {
    HotkeyBinding binding;
#ifdef _WIN32
    binding.virtualKey = VK_F2;
#endif
    return binding;
}
...
HotkeyBinding ClickContinuousInputHotkeySettings::binding() {
    QSettings settings;
...
void ClickContinuousInputHotkeySettings::setBinding(const HotkeyBinding& binding) {
    QSettings settings;
...
bool ClickContinuousInputHotkeySettings::isDefaultBinding(const HotkeyBinding& binding) {
    const HotkeyBinding defaultValue = defaultBinding();
    return binding.virtualKey == defaultValue.virtualKey && !binding.ctrl && !binding.alt
           && !binding.shift;
}
