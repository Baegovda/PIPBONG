...
MainWindow* g_foregroundSyncMainWindow = nullptr;
...
void CALLBACK foregroundWindowEventProc(HWINEVENTHOOK,
                                        ...
                                        DWORD,
                                        DWORD) {
    if (event != EVENT_SYSTEM_FOREGROUND || !g_foregroundSyncMainWindow) {
        return;
    }
    QMetaObject::invokeMethod(g_foregroundSyncMainWindow,
...
MainWindow::MainWindow(QWidget* parent)
    ...
    g_foregroundSyncMainWindow = this;
...
MainWindow::~MainWindow() {
    ...
    if (g_foregroundSyncMainWindow == this) {
        g_foregroundSyncMainWindow = nullptr;
    }
#endif
    stopAllSessions();
...
void MainWindow::showEvent(QShowEvent* event) {
    ...
    if (!m_initialForegroundSyncDone) {
        ...
        QTimer::singleShot(0, this, &MainWindow::onForegroundWindowChanged);
    }
...
void MainWindow::launchHoldKeyTapRun(FeatureRunSession& session, Feature* feature, int virtualKey) {
    ...
    CrashReporter::noteBreadcrumb(
        QStringLiteral("run"),
        QStringLiteral("hold tap \"%1\" vk=%2")
            .arg(QString::fromStdString(feature->name()))
            .arg(virtualKey));
...
void MainWindow::loadActiveProfile(bool quiet, bool scheduleTriggerRestore) {
    if (!m_profileManager) {
        return;
    }
...
void MainWindow::setProfileSwitchUiLocked(bool locked) {
    if (m_profileList) {
        m_profileList->setReorderEnabled(!locked);
        m_profileList->setFeatureDropEnabled(!locked);
    }
    if (m_featureList) {
        m_featureList->setListDragEnabled(!locked);
    }
}
...
void MainWindow::abortProfileSwitchPipeline(bool resyncSelection) {
...
void MainWindow::requestAutoProfileSwitch(const QString& targetProfileId,
    ...
    m_pendingAutoSwitchStabilityMs = stabilityMs >= 0 ? stabilityMs : 500;
    m_pendingAutoSwitchMinIntervalMs = minIntervalMs >= 0 ? minIntervalMs : 800;
    if (!m_autoProfileSwitchStabilityTimer) {
        return;
    }
...
void MainWindow::syncProfileToForegroundWindow() {
    ...
    if (fallingBackToDefault) {
        constexpr int kDefaultFallbackStableMs = 250;
...
MainWindow::TargetDetailHwndResolve MainWindow::resolveTargetDetailDisplayHwnd(
    ...
    TargetDetailHwndResolve result;

    const auto classifyHwnd = [&](HWND hwnd) -> std::optional<bool> {
        if (!hwnd || !IsWindow(hwnd)) {
            return std::nullopt;
        }
        wchar_t titleBuffer[512]{};
        ...
        if (!subBinding.isEmpty()
            && foregroundMatchesScopedSubTarget(title,
                                                hwnd,
                                                mainBinding,
                                                subBinding,
                                                mainProcessPath,
        ...
        if (!mainBinding.isEmpty()
            && foregroundMatchesScopedMainTarget(title,
                                                 hwnd,
                                                 mainBinding,
                                                 subBinding,
                                                 mainProcessPath,
        ...
        if (!subBinding.isEmpty()
            && foregroundHwndMatchesLinkedProcess(hwnd, {}, subProcessPath)) {
            return true;
        }
        if (!mainBinding.isEmpty()
            && foregroundHwndMatchesLinkedProcess(hwnd, mainProcessPath, {})) {
            return false;
        }
        return std::nullopt;
    };

    const auto applyHwnd = [&](HWND hwnd, bool isSub) {
        result.hwnd = hwnd;
        result.isSubTarget = isSub;
    };

    HWND fgHwnd = foregroundRootHwnd();
    if (fgHwnd && !isPipbongProcessForeground(fgHwnd)) {
        if (const std::optional<bool> role = classifyHwnd(fgHwnd)) {
            applyHwnd(fgHwnd, *role);
            return result;
        }
    }

    if (fgHwnd && isPipbongProcessForeground(fgHwnd) && m_lastProfileLinkedForegroundHwnd
        && IsWindow(m_lastProfileLinkedForegroundHwnd)) {
        applyHwnd(m_lastProfileLinkedForegroundHwnd, m_lastProfileLinkedForegroundIsSub);
        return result;
    }

    if (HWND hintHwnd = ScreenCapture::foregroundHintWindow()) {
        if (const std::optional<bool> role = classifyHwnd(hintHwnd)) {
            applyHwnd(hintHwnd, *role);
            return result;
        }
    }

    const HWND mainHwnd =
        mainBinding.isEmpty() ? nullptr : findVisibleTopLevelWindowHwnd(mainBinding, mainProcessPath);
    const HWND subHwnd =
        subBinding.isEmpty() ? nullptr : findVisibleTopLevelWindowHwnd(subBinding, subProcessPath);
    ...
    if (mainHwnd && subHwnd) {
        if (mainHwnd == subHwnd) {
            if (const std::optional<bool> role = classifyHwnd(mainHwnd)) {
                applyHwnd(mainHwnd, *role);
            } else {
                applyHwnd(mainHwnd, false);
            }
            return result;
        }
        const bool mainMin = IsIconic(mainHwnd) != FALSE;
        const bool subMin = IsIconic(subHwnd) != FALSE;
        if (mainMin && !subMin) {
            applyHwnd(subHwnd, true);
...
QString MainWindow::resolveProfileIdForForeground(HWND hwnd,
                                                  const QString& foregroundTitle) const {
    if (!m_profileManager) {
        return {};
    }
    ...
    QString target;
    if (!foregroundTitle.isEmpty()) {
        target = m_profileManager->profileIdForForegroundTitle(foregroundTitle);
    }
    ...
    if (target.isEmpty()) {
        target = m_profileManager->defaultProfileId();
    }
    return target;
}
...
void MainWindow::autoProfileSwitchTimingForTarget(HWND hwnd,
    ...
    constexpr int kDefaultStabilityMs = 500;
    constexpr int kDefaultMinIntervalMs = 800;
    constexpr int kLinkedTitleStabilityMs = 120;
    constexpr int kLinkedTitleMinIntervalMs = 350;
    constexpr int kProcessMatchStabilityMs = 0;
    constexpr int kProcessMatchMinIntervalMs = 150;
    constexpr int kDefaultFallbackStabilityMs = 120;
    constexpr int kDefaultFallbackMinIntervalMs = 600;

    int stabilityMs = kDefaultStabilityMs;
    int minIntervalMs = kDefaultMinIntervalMs;
    if (m_profileManager && !targetProfileId.isEmpty()) {
        if (m_profileManager->isDefaultProfile(targetProfileId)) {
            stabilityMs = kDefaultFallbackStabilityMs;
            minIntervalMs = kDefaultFallbackMinIntervalMs;
        } else if (isDefinitiveProcessProfileMatch(hwnd, targetProfileId)) {
            stabilityMs = kProcessMatchStabilityMs;
            minIntervalMs = kProcessMatchMinIntervalMs;
        } else {
            stabilityMs = kLinkedTitleStabilityMs;
            minIntervalMs = kLinkedTitleMinIntervalMs;
        }
    }
    if (stabilityMsOut) {
...
QString MainWindow::profileIdForForegroundHwnd(HWND hwnd) const {
    ...
    QString bestId;
    int bestScore = 0;
    for (const ProfileManager::Profile& profile : m_profileManager->profiles()) {
        if (m_profileManager->isDefaultProfile(profile.id)) {
            continue;
        }
        const QString mainProc = m_profileManager->linkedTargetProcessPath(profile.id);
        const QString subProc = m_profileManager->subLinkedTargetProcessPath(profile.id);
...
bool MainWindow::resolveForegroundTargetProfileId(HWND hwnd,
    ...
    if (isPipbongProcessForeground(hwnd) || isShellTransientForegroundWindow(hwnd)) {
        return false;
    }
    hwnd = GetAncestor(hwnd, GA_ROOT);
    if (!hwnd || !IsWindow(hwnd)) {
    ...
    if (foregroundTitleOut) {
        *foregroundTitleOut = foregroundTitle;
    }
    ...
    *targetProfileIdOut = targetProfileId;
    return true;
#else
    Q_UNUSED(hwnd);
...
bool MainWindow::switchToForegroundLinkedProfileIfNeeded(bool forceImmediate) {
    ...
    const QString targetProfileId = !foregroundTitle.isEmpty()
                                        ? m_profileManager->profileIdForForegroundTitle(
                                              foregroundTitle)
                                        : profileIdForForegroundHwnd(hwnd);
...
void MainWindow::updateTargetWindowDetails() {
    ...
    if (m_profileManager) {
        const QString profileId = m_profileManager->activeProfileId();
        linkedProcessPath = m_profileManager->linkedTargetProcessPath(profileId);
        subProcessPath = m_profileManager->subLinkedTargetProcessPath(profileId);
    }
    ...
    const TargetDetailHwndResolve resolved =
        resolveTargetDetailDisplayHwnd(savedTitle, subBinding, linkedProcessPath, subProcessPath);
    HWND hwnd = resolved.hwnd;
    bool detailIsSubTarget = resolved.isSubTarget;

    if (!sessionOwnsCapture && hwnd && IsWindow(hwnd)) {
        ScreenCapture::setTargetWindow(hwnd);
        if (detailIsSubTarget) {
            ScreenCapture::setForegroundHintWindow(hwnd);
        }
    }
    ...
    if (!hwnd || !IsWindow(hwnd)) {
        if (savedTitle.isEmpty() && subBinding.isEmpty()) {
            ...
            const HWND subHwnd = subBinding.isEmpty()
                                       ? nullptr
                                       : findVisibleTopLevelWindowHwnd(subBinding, subProcessPath);
            const HWND mainHwnd =
                savedTitle.isEmpty() ? nullptr
                                     : findVisibleTopLevelWindowHwnd(savedTitle, linkedProcessPath);
            if (subHwnd && !mainHwnd) {
                hwnd = subHwnd;
                detailIsSubTarget = true;
            } else if (mainHwnd && !subHwnd) {
                hwnd = mainHwnd;
                detailIsSubTarget = false;
            } else {
                const bool storedSubTarget = savedTitle.isEmpty() && !subBinding.isEmpty();
                const QString displayTitle = storedSubTarget ? subBinding : savedTitle;
                const QString& processPathForDisplay =
                    storedSubTarget ? subProcessPath : linkedProcessPath;
                const QString processName = processPathForDisplay.isEmpty()
                                                ? QString()
                                                : QFileInfo(processPathForDisplay).fileName();
                m_targetWindowDetailPanel->showStoredTargetBinding(displayTitle,
                                                                   processName,
                                                                   processPathForDisplay,
                                                                   storedSubTarget);
                return;
            }
        }
    ...
    const bool isSubTarget = detailIsSubTarget
